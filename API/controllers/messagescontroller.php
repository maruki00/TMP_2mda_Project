...
class messagescontroller extends Controller{
    ...
    public function getall(){
        if(isset($_POST['token'])){
            if(userscontroller::isAuth($_POST['token'])){
                $data = Helper::parseAccessToken($_POST['token']);
                $this->message=messagesmodel::exec("select user_id_from,message,messaged_date,user_id_to,
                            (select fullname from users where id   = m.user_id_from) as user
                             ,(select imagepath from users where id= m.user_id_from) as userimage 
                             from messages m where (user_id_from=".$data[0]." or user_id_to=".$data[0].")  
                                                              group by m.user_id_from 
                                                              Desc having m.user_id_from!=".$data[0]);
                if($this->message){
